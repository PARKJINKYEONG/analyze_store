...
def load_store_grid_count():
    path = os.path.join(OUT_DIR, "store_grid_count.csv")

    if not os.path.exists(path):
        print("store_grid_count.csv 없음: 업소 격자 수는 표시하지 않습니다.")
        return pd.DataFrame()
    ...
    df["xcdn"] = pd.to_numeric(df["xcdn"], errors="coerce")
    df["ycdn"] = pd.to_numeric(df["ycdn"], errors="coerce")
    df["store_count"] = pd.to_numeric(df["store_count"], errors="coerce").fillna(0)

    df = df.dropna(subset=["cell_id", "xcdn", "ycdn"])
...
def add_grid_heat_layer(m, cell, categories, target_dongs=None, layer_name="격자 소비 밀집도", store_grid=None):
    ...
    if store_grid is not None and not store_grid.empty:
            sg = store_grid[store_grid["category"].isin(categories)].copy()

            if target_dongs is not None:
                sg = sg[sg["dong"].isin(target_dongs)].copy()

            sg = (
                sg.groupby(["cell_id"], as_index=False)
                .agg(grid_store_count=("store_count", "sum"))
            )

            grid = grid.merge(sg, on="cell_id", how="left")
            grid["grid_store_count"] = grid["grid_store_count"].fillna(0)
    else:
            grid["grid_store_count"] = 0
...
def make_activation_map(boundary, summary, map_name, cell, categories, store_grid=None):
    gdf = boundary.merge(
        summary,
        on="dong_key",
        how="left",
